...
export class GeradorMissoes {
	suspeito: ISuspeito[]
	duracao:number
	tempoInicio:number
	segundos:number

	constructor ( public suspeitos:ISuspeito[], duracao:number = 5, tempoInicio:number = 0, segundos:number = 0 ) {
	...
	Missao () {
		...
		rl.question('Selecione E - para ir a um estabelecimento ou P - digitar pistas e capturar suspeito  ', (resposta: string) => {
			switch (resposta) {
				case 'E':
					rl.question('Escolha seu destino: \n A: Biblioteca \n B: Restaurante \n C: Aeroporto \n (obs: para escolher uma alteranativa digite apenas a letra e a mesma deverá ser maiúscula) \n digite a alternativa: ', (destino:string) => {
						switch (destino) {
							case 'A':
	...
	prenderSuspeito (listaPerguntasSuspeito:number = 0) {
		...
		rl.question('Digite R: responder ou V: voltar as pistas: ', (resposta:string) => {
			switch (resposta) {
				case 'R':
					rl.question('Digite a alternativa correta: ', (alternativa:string) => {
	...
	enviaPerguntas (perguntas: ISuspeito) {
